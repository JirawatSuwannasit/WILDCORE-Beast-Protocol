...
of {ANIMATED_ELEMENT}, ladder, checkpoint lantern (unlit + lit).
...
transparent background. Cozy-menacing critter robots matching each stage:
...
```
M4.{N}: Build the complete {STAGE} stage per GDD §3.{X}, same structural
standard as Speedway (checkpoints, telegraphs, camera, boss door ritual).
- Signature gimmicks: {GIMMICKS}
- Enemies: {ENEMIES} - all telegraphed >=20 frames.
- Mid-boss: {MIDBOSS}.
