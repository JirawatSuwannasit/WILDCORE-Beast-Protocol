...
- [ ] Vertical path ≥ 35% of total; report the exact %.
...
- [ ] Ground surface is NOT a monotonic slope: report per-screen surface
      ...
      variation.
- [ ] Contains a real wall-kick **ASCENT SHAFT** (2+ screens tall, facing kick
      walls, vertical camera) — state its screen range.
- [ ] Contains a controlled **DESCENT** (steerable, sees ≥ half a screen below,
      no blind drops onto hazards) — state its screen range.
- [ ] Contains a **MULTI-FLOOR ROOM** (stacked layers, player picks a layer) —
      state its screen range.
