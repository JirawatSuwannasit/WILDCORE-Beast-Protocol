...
- **Route shape (anti-corridor rule):** stages must NOT read as a flat left-to-right line. The goal is that **no two stages share the same route shape** — each stage picks a **dominant axis** that fits its theme (e.g. Speedway = horizontal-dominant high-speed; Reservoir = vertical-dominant descent). Variety across the 8 stages matters more than every stage hitting the same vertical quota.
  - **Dominant axis (declared per stage in §3):** each stage states whether it is horizontal-dominant, vertical-dominant, or mixed. Horizontal-dominant stages are allowed; they just cannot be a flat corridor.
  - **Anti-corridor floor (applies to ALL stages regardless of axis):** never more than 3 consecutive screens moving in the same direction; the dominant direction changes at least 4 times per stage; the ground surface must NOT be a monotonic slope (real elevation changes, layered platforms, not one long ramp).
  - **Vertical content (scaled to axis):** vertical-dominant stages target ≥ 35% vertical path; horizontal-dominant stages target ≥ 20% and are not required to be tall. Report the actual % either way.
  - **Structural elements — at least TWO of these three per stage**, chosen to fit the theme (not all three forced on every stage): a wall-kick **ascent shaft** (2+ screens tall), a controlled **descent** (readable landings, no blind drops onto hazards), a **multi-floor room** (stacked platform layers where the player picks a layer). A stage's §3 "Route shape" spec names which it uses.
  - **Branch & rejoin (MANDATORY, every stage — this is the core of route variety):** at least one fork on the main path — e.g., **upper route = riskier + pickups, lower route = safer + slower** — both rejoining before the next beat. Secrets branch off the less obvious side.
...
### 2.7 Per-Stage Build & Verification Checklist (mandatory report per stage PR)
Every stage PR MUST include a machine-checkable report proving these. A stage
is NOT done until all pass. (Added after M4.1 Reservoir shipped a repetitive,
single-corridor first draft despite §2.6 — the rules existed but weren't
verifiable; §2.7 makes them measurable, not just aspirational.)

**Terrain shape (anti-corridor):**
- [ ] Declare the stage's DOMINANT AXIS (horizontal / vertical / mixed) and
      confirm it matches the §3 "Route shape" spec — and that it differs from
      the neighbouring stages built so far (no two stages the same shape).
- [ ] Vertical path meets the axis target (vertical-dominant ≥ 35%,
      horizontal-dominant ≥ 20%); report the exact %.
- [ ] Dominant direction changes ≥ 4 times; report the count.
- [ ] No run of > 3 consecutive screens moving the same direction; report the
      longest run.
- [ ] Ground surface is NOT a monotonic slope: report per-screen surface
      height; no more than 3 consecutive screens with near-zero vertical
      variation.
- [ ] Contains at least TWO of the three structural elements (chosen to fit the
      theme, per the stage's §3 route shape), and names which:
      • wall-kick **ASCENT SHAFT** (2+ screens tall, facing kick walls,
        vertical camera) — state screen range if used.
      • controlled **DESCENT** (steerable, sees ≥ half a screen below, no blind
        drops onto hazards) — state screen range if used.
      • **MULTI-FLOOR ROOM** (stacked layers, player picks a layer) — state
        screen range if used.
- [ ] Contains a **BRANCH & REJOIN** (upper riskier+pickups / lower safer+slower,
      rejoining before the next beat) per the stage's §3 route shape — state
      the fork/rejoin screens. (Mandatory for every stage.)

**Content variety (anti-formula):**
- [ ] No two consecutive screens share an identical (enemy + hazard)
      signature; report any that do.
- [ ] Encounter density averages 1 per 1.5–2 screens (not every screen packed,
      not long empty runs); report encounters per beat.
- [ ] The stage's signature gimmick appears as a through-line across ALL beats
      including setpiece and finalExam — NOT dropped after the first half;
      report which screens use it.
- [ ] Beat 5 introduces a genuine SECOND gimmick/variation (not a repeat of the
      beat-2 tutorial).
- [ ] finalExam (beat 8) is demonstrably the hardest COMBINATION, distinct from
      escalation (beat 3) — one line on how it differs.
- [ ] Never two hazard TYPES introduced in the same room.

**Fairness (unchanged pillars, restated for stages):**
- [ ] Every gap/jump clearable with the base kit (run + variable jump max 3.5
      tiles + wall-kick), no dash required, with margin; report max gap width.
- [ ] Gated secrets show a readable "locked" hint; reachable WITH the gate,
      not without.
- [ ] Playable boundaries: solid walls at dead-ends and outer bounds; true
      end-of-stage reads as intentionally terminating.
...
- **Route shape (HORIZONTAL-DOMINANT — high-speed; the counterpoint to Reservoir's vertical descent):** branch at the turbine tower (upper blade-platform route = risky + pickups / lower fence corridor = safe) rejoining at the mid-boss; high-speed downhill boost-strip **descent** as the setpiece; multi-floor highway underpass breather. Uses DESCENT + MULTI-FLOOR as its two structural elements (a tall ascent shaft is NOT required here — keep the pace horizontal). Short wall-kick climbs are fine as texture but the stage should read as fast and flat-ish with elevation spikes, not a climb.
...
## 3b. Enemy Roster & Hazard Matrix

### Universal enemy rules
- Contact damage 1–2 units; every ranged/melee attack has a ≥20-frame telegraph (feel pillar #4).
- Enemies respawn on checkpoint reset and on re-entering a screen from Stage Select.
- Drops: 15% small energy, 5% large energy, 10% weapon energy small (only when a boss weapon is owned).
- Names match the art pack files `enemies_{stage}.png` (Playbook A4).

### Regular enemies (2–3 per stage + mid-boss)
| Stage | Enemy | HP | Behavior & Telegraph |
|---|---|---|---|
| Savanna | Patrol Drone | 2 | Hovers a fixed lane; fires 1 bolt when the player crosses its sightline (lens flashes 20f first). |
| Savanna | Spark Bug | 1 | Hops toward the player in readable arcs; sparks grow just before each hop. Contact only. |
| Savanna | Turret Sunflower | 3 | Rooted; petals open (24f) → 3-way spread shot → closes. Invulnerable while closed. |
| Reservoir | Bubble Crab | 2 | Walks floor/walls inside a bubble shield; pop the bubble (any shot) → 2s vulnerable, then re-bubbles. |
| Reservoir | Dart Fish | 1 | Idles in water; wiggles 20f, then dashes in a straight line. Dies to one hit but fast. |
| Foundry | Slag Blob | 2 | Crawls; periodically inflates (glow 24f) and lobs 2 slag arcs. Slag leaves brief floor flames. |
| Foundry | Ember Bat | 1 | Ceiling sleeper; eyes light 20f → swoop in a U-curve when the player passes below. |
| Observatory | Snowball Roller | 2 | Pushes a growing snowball down slopes; the ball (blockable, breakable) hits for 2, the bot for 1. |
| Observatory | Frost Wisp | 1 | Slow float toward player; carries a chill aura (slows run 30% inside radius, visible ring). |
| Skyhaven | Kite Drone | 1 | Rides gust cycles; aligns above the player, flashes 20f, dive-bombs along a shown dotted line. |
| Skyhaven | Cloud Puff Mine | 1 | Drifts on wind; proximity fuse — puffs up + blinks 24f before a small burst (outrunnable). |
| Greenhouse | Seed Spitter | 2 | Rooted; lobs seeds on an arc, seeds sprout thorn patches that wilt after 4s. |
| Greenhouse | Vine Crawler | 2 | Patrols flowering vines/walls; lunges one tile when the player is adjacent (rears up 20f first). |
| Quarry | Rock Beetle | 3 | Armored face — frontal shots plink off; vulnerable from behind/above or to charged shots. Slow charge attack. |
| Quarry | Crystal Bat | 1 | Dark-zone swooper; crystal glint 20f betrays position before the swoop. |
| Eclipse | Lantern Ghost Bot | 2 | Phases in only inside lantern light; lantern flickers 24f before it materializes and lobs an ember. |
| Eclipse | Rooftop Cat Drone | 2 | Pounces between AC units toward the player; tail-light blinks 20f pre-pounce; brief crouch after landing = punish window. |

*(Mid-bosses per stage are specified in §3; art in Playbook A4.)*

### Hazard matrix (obstacles per stage)
| Hazard | Found in | Damage | Rule |
|---|---|---|---|
| Spikes | Savanna, Quarry, others | Lethal | Always visible before reachable; Legs Capsule absorbs 1 hit per checkpoint. |
| Bottomless pit | Skyhaven, Eclipse rooftops | Lethal | Edge markers (circling birds / sign glow) telegraph every pit. |
| Electric fence | Savanna | 2 | Pulses on a visible+audible beat; safe windows ≥40f. |
| Rising/standing lava | Foundry | 4 + knockback | Chase speed tuned in config; never faster than dash-less run. |
| Piston crusher | Foundry | Lethal | Rails visible, 2-cycle rhythm, ≥30f open window. |
| Icicle fall | Observatory | 2 | Shakes 20f before dropping; becomes a platform after landing (some). |
| Toxic pool | Greenhouse | 1 per 0.5s (DoT) | Never instant death; exit always reachable within 2s. |
